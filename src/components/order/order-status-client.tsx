...
import { getDish, getVariant } from "@/data/menu";
...
const STATUS_STEPS: OrderStatus[] = ["new", "cooking", "ready", "served"];
...
export function OrderStatusClient({ id }: { id: string }) {
  ...
  const fetchOrder = useCallback(async () => {
    try {
      ...
      if (!res.ok) return;
      setOrder((await res.json()) as ApiOrder);
      setState("ready");
    } catch {
  ...
  useEffect(() => {
    // Initial load; state is only set after the awaited fetch resolves.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    fetchOrder();
  }, [fetchOrder]);

  useEffect(() => {
    if (state !== "ready" || order?.status === "served") return;
    const timer = setInterval(fetchOrder, 5000);
    return () => clearInterval(timer);
  }, [state, order?.status, fetchOrder]);

  function itemLabel(item: ApiItem): string {
    const dish = getDish(item.slug);
    const base = dish ? pick(dish.name, locale) : item.name;
    if (!item.variantId) return base;
    const variant = dish ? getVariant(dish, item.variantId) : undefined;
    const variantName = variant ? pick(variant.name, locale) : item.variantName;
            ...
            <div className="ornamental-border bg-surface-container-low p-6 sm:p-8">
              ...
              <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
                ...
                <span className="font-body text-sm text-on-surface-variant">
                  {t("order.table", { table: order.table })} ·{" "}
                  {t("order.number", { number: order.id })}
                </span>
              </div>
